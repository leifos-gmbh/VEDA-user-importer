...
class ilVedaConnector
{
    ...
    /**
     * @param string|null $oid
     * @return \Swagger\Client\Model\AufsichtspersonKurszugriff[]
     * @throws \ilVedaConnectionException
     */
    public function readTrainingCourseTrainSupervisors(?string $oid)
    {
        if(!$this->api_training_course_train instanceof AusbildungszgeApi)
        {
            list(
        ...
        try {
            $response = $this->api_training_course_train->getAufsichtspersonenVonAusbildungszugUsingGET($oid);
            return $response;
        }
        catch(ApiException $e) {
            $this->logger->error('getAufsichtspersonenVonAusbildungszugUsingGET failed with message: ' . $e->getMessage());
            $this->logger->dump($e->getResponseHeaders(), \ilLogLevel::WARNING);
            $this->logger->dump($e->getTraceAsString(), \ilLogLevel::WARNING);
            $this->logger->warning($e->getResponseBody());
            ...
            $this->logger->warning('getAufsichtspersonenVonAusbildungszugUsingGET failed with message: ' . $e->getMessage());
            throw new \ilVedaConnectionException($e->getMessage(), \ilVedaConnectionException::ERR_API);
        }
    }
